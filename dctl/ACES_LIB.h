...
(_logf(aces.x) + 4.645361f) / 4.1865183f;
aces.y =   aces.y < 0.00130127f ? (aces.y - 0.00130127f) / 0.04911331f :
aces.y < 0.001897934f ? (_logf(aces.y) + 6.644415f) / 37.74261f : 
aces.y < 0.118428f ? _logf(aces.y) * (0.02871031f * _logf(aces.y) + 0.383914f) + 1.288383f : 
(_logf(aces.y) + 4.645361f) / 4.1865183f;
aces.z =   aces.z < 0.00130127f ? (aces.z - 0.00130127f) / 0.04911331f :
aces.z < 0.001897934f ? (_logf(aces.z) + 6.644415f) / 37.74261f : 
aces.z < 0.118428f ? _logf(aces.z) * (0.02871031f * _logf(aces.z) + 0.383914f) + 1.288383f : 
(_logf(aces.z) + 4.645361f) / 4.1865183f;
return aces;
}
...
c = c + in;
const float discrim = _sqrtf( b * b - 4.0f * a * c);
...
__DEVICE__ float3 rrt_sweeteners( float3 in, bool do_glow, bool do_redmod, bool do_desat) {
float3 aces = in;
...
aces = max_f3_f( aces, 0.0f);
rgbPre = mult_f3_f33( aces, mult_f33_f33( RGBtoXYZ( AP0), XYZtoRGB( AP1)));
...
float3 aces = mult_f3_f33( rgbPost, mult_f33_f33( RGBtoXYZ( AP1), XYZtoRGB( AP0)));
aces = max_f3_f( aces, 0.0f);
float hue = rgb_2_hue( aces);
float centeredHue = center_hue( hue, RRT_RED_HUE);
float hueWeight = cubic_basis_shaper( centeredHue, RRT_RED_WIDTH);
...
aces.x = ( -b - _sqrtf( b * b - 4.0f * a * c)) / ( 2.0f * a);
float saturation = rgb_2_saturation( aces);
...
float3 rgbPre = rrt_sweeteners( in, GLOW, REDMOD, DESAT);
...
float3 rgbPre = linCV_2_Y_f3( linearCV, 48.0f, _exp10f(_log10f(0.02f)));
float3 rgbPost;
rgbPost = segmented_spline_c9_rev_f3( rgbPre);
float3 oces = mult_f3_f33( rgbPost, mult_f33_f33( RGBtoXYZ( AP1), XYZtoRGB( AP0)));
return oces;
}
...
ych.y = _sqrtf(yo);
ych.z = _atan2f(yab.z, yab.y) * (180.0f / 3.141592653589793f);
...
float3 rgbOces = mult_f3_f33( rgbPost, mult_f33_f33( RGBtoXYZ( AP1), XYZtoRGB( AP0)));
return rgbOces;
}
...
float3 linearCV;
linearCV.x = Y_2_linCV( rgbPost.x, 48.0f, _exp10f(_log10f(0.02f)));
linearCV.y = Y_2_linCV( rgbPost.y, 48.0f, _exp10f(_log10f(0.02f)));
linearCV.z = Y_2_linCV( rgbPost.z, 48.0f, _exp10f(_log10f(0.02f)));
...
linearCV.y = Y_2_linCV( rgbPost.y, 48.0f, _exp10f(_log10f(0.02f)));
linearCV.z = Y_2_linCV( rgbPost.z, 48.0f, _exp10f(_log10f(0.02f)));
linearCV.x = _fminf( linearCV.x, 1.0f) * SCALE;
linearCV.y = _fminf( linearCV.y, 1.0f) * SCALE;
linearCV.z = _fminf( linearCV.z, 1.0f) * SCALE;
linearCV = darkSurround_to_dimSurround( linearCV);
linearCV = mult_f3_f33( linearCV, calc_sat_adjust_matrix( ODT_SAT_FACTOR, make_float3(RGBtoXYZ( AP1).c0.y, RGBtoXYZ( AP1).c1.y, RGBtoXYZ( AP1).c2.y)));
float3 XYZ = mult_f3_f33( linearCV, RGBtoXYZ( AP1));
linearCV = mult_f3_f33( XYZ, XYZ_2_DISPLAY_PRI_MAT);
...
rgbPost = add_f_f3( -_exp10f(-4.4550166483f), rgbPost);
...
linearCV.x = _fminf( linearCV.x, NEW_WHT) * SCALE;
linearCV.y = _fminf( linearCV.y, NEW_WHT) * SCALE;
linearCV.z = _fminf( linearCV.z, NEW_WHT) * SCALE;
float3 XYZ = mult_f3_f33( linearCV, RGBtoXYZ( AP1));
linearCV = mult_f3_f33( XYZ, XYZ_2_DISPLAY_PRI_MAT);
linearCV = clamp_f3( linearCV, 0.0f, 1.0f);
...
float3 rgbPre;
rgbPre.x = linCV_2_Y( linearCV.x, 48.0f, _exp10f(_log10f(0.02f)));
rgbPre.y = linCV_2_Y( linearCV.y, 48.0f, _exp10f(_log10f(0.02f)));
rgbPre.z = linCV_2_Y( linearCV.z, 48.0f, _exp10f(_log10f(0.02f)));
float3 rgbPost;
...
rgbPre = add_f_f3( _exp10f(-4.4550166483f), rgbPre);
float3 rgbPost;
