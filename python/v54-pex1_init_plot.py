...
def findGamutBoundaryIntersection(JMh_s, JM_cusp, J_focus, J_max, slope_gain, smoothness, gamma_top, gamma_bottom):
    ...
    JM_cusp0 = JM_cusp[0] * (1.0 + 0.055 * s)   # J
    JM_cusp1 = JM_cusp[1] * (1.0 + 0.183 * s)   # M
...
#         break
  ...
  for i in range(gamutCuspTableSize):
    ...
    # move cusp by the same amount it is moved by when smoothCusps is applied
#     JMcusp[0] *= 1.0 + 0.055 * smoothCusps
#     JMcusp[1] *= 1.0 + 0.183 * smoothCusps
...
#   print(format_array3(gamutCuspTableReach, "__CONSTANT__ float3 gamutCuspTableReach[{}]".format(gamutCuspTableSize), 3))
  print(format_array(np.split(gamutCuspTableReach, 3, 1)[0].reshape(-1), "__CONSTANT__ float3 gamutCuspTableReach[{}]".format(gamutCuspTableSize), 3))
...
#   print(format_array3(cgamutReachTable, "__CONSTANT__ float3 cgamutReachTable[{}]".format(gamutCuspTableSize), 3))
  print(format_array(np.split(cgamutReachTable, 3, 1)[0].reshape(-1), "__CONSTANT__ float3 cgamutReachTable[{}]".format(gamutCuspTableSize), 3))
