...
# by an Output Transform implementation.  
#
...
#
# Usage:
# > python compare_ot_test_frames.py <PATH-TO-REFERENCE-IMAGES> <PATH-TO-ACTUAL-IMAGES>
...
#     avoid_warning_cmd = '-o ' + os.path.join(tempfile.gettempdir(), 'tmp.exr') + ' '
    avoid_warning_cmd = '-o ' + os.path.join(tempfile.gettempdir(), 'tmp.tif') + ' '

#     oiio_cmd = base_cmd + box_cmd + nan_err_cmd + range_cmd + avoid_warning_cmd
    oiio_cmd = base_cmd + nan_err_cmd + range_cmd + avoid_warning_cmd
