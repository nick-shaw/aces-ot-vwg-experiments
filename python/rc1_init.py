...
smoothM = 0.27
lowerHullGamma = 1.14
compressionFuncParams = [0.75, 1.1, 1.3, 1.0]
...
def JMh_to_limit_RGB(JMh):
  luminanceXYZ = Hellwig2022_JMh_to_XYZ( JMh, limitWhite, surround, L_A, Y_b)
...
def JMh_to_reach_RGB(JMh):
   luminanceXYZ = Hellwig2022_JMh_to_XYZ( JMh, inWhite, surround, L_A, Y_b)
...
def findGamutBoundaryIntersection(JMh_s, JM_cusp, J_focus, J_max, slope_gain, smoothness, gamma_top, gamma_bottom):
    ...
    JM_cusp0 = JM_cusp[0] * (1.0 + smoothJ * s)   # J
    JM_cusp1 = JM_cusp[1] * (1.0 + smoothM * s)   # M
    newCusp = [JM_cusp0, JM_cusp1]

    J_intersect_source = solve_J_intersect(JM_source, J_focus, J_max, slope_gain)
    J_intersect_cusp = solve_J_intersect(newCusp, J_focus, J_max, slope_gain)
    ...
    M_boundary_lower = J_intersect_cusp * pow(J_intersect_source / J_intersect_cusp, 1 / gamma_bottom) / (newCusp[0] / newCusp[1] - slope)

    M_boundary_upper = newCusp[1] * (J_max - J_intersect_cusp) * pow((J_max - J_intersect_source) / (J_max - J_intersect_cusp), 1.0 / gamma_top) / (slope * newCusp[1] + J_max - newCusp[0])

    M_boundary = newCusp[1] * smin(M_boundary_lower / newCusp[1], M_boundary_upper / newCusp[1], s)
...
def init():
  global peakLuminance, primariesLimit, whiteLimit, inWhite, outWhite, boundaryRGB, RGB_to_XYZ_limit, limitWhite
...
#   refWhite = vector_dot(RGB_to_XYZ_limit, white)
  limitWhite = vector_dot(RGB_to_XYZ_limit, white) # same as refWhite
...
def getFocusGain(J, cuspJ):
     thr = lerp(cuspJ, limitJmax, focusGainBlend);
     if (J > thr):
       # Approximate inverse required above threshold
       gain = (limitJmax - thr) / max(0.0001, (limitJmax - min(limitJmax, J)))
       return pow(np.log10(gain), 1.0 / focusAdjustGain) + 1.0
     else:
...
def print_constants():
  ...
  print(format_array3(CAT_CAT16, "__CONSTANT__ float3x3 MATRIX_16"))
  print(format_array3(np.linalg.inv(CAT_CAT16), "__CONSTANT__ float3x3 MATRIX_INVERSE_16"))
  ...
  print("__CONSTANT__ float3 inWhite = " + format_vector(inWhite*100) + ";")
  print("__CONSTANT__ float3 outWhite = " + format_vector(outWhite*100) + ";")
  print("__CONSTANT__ float3 limitWhite = " + format_vector(limitWhite*100) + ";")
  print()
...
#     XYZ = vector_dot(RGB_to_XYZ_reach, np.array([0., 0., 16384.]))
#     JMh = XYZ_to_Hellwig2022_JMh(100 * XYZ, inWhite, L_A, Y_b, surround)
#     print("JMh = ", JMh)
#     Y = J_to_Y(JMh[0], L_A, Y_b, surround[1])
#     print("Y = ", Y /100)
#     r_hit_max = 896.0
#     print(Y_to_J(100 * r_hit_max, L_A, Y_b, surround[1]))
#     hues = np.arange(360)
